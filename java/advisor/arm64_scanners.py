...
class Arm64Scanners:
    ...
    def __init__(self, issue_type_config, output_format, arch, march):
        ...
        """
        self.scanners = [
                         Arm64JarScanner(output_format=output_format, arch=arch, march=march),
                         Arm64PomScanner(output_format=output_format, arch=arch, march=march),
                         Arm64JavaSourceScanner(output_format=output_format, arch=arch, march=march)
                        ]
